...
Exibe todos os Services no mesmo namespace. Permite conferir endereço (ClusterIP), porta exposta e mapeamento para os Pods backend/frontend.
```
```bash
kubectl get pods -A
...
Captura logs do Pod específico <podname>. Útil para rastrear falhas pontuais ou mensagens de erro num nó determinado.
```
```bash
...
```
```bash
kubectl exec -it $(kubectl get pod -n app -l app=backend -o \
  jsonpath='{.items[0].metadata.name}') -n app -- sh
